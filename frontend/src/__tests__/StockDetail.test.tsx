...
describe('StockDetail', () => {
  ...
  it('로딩 중에 스피너(로딩 텍스트)를 표시한다', () => {
    ...
    (fetchRecommendationDetail as ReturnType<typeof vi.fn>).mockReturnValue(
      new Promise(() => {}),
    );
    render(<StockDetail krxCode="005930" onClose={() => {}} />);
  ...
  it('reasoning 텍스트를 표시한다', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('삼성전자는 반도체 수요 회복으로 강한 매수 신호가 감지됩니다.'),
      ).toBeInTheDocument();
    });
  });
  ...
  it('닫기 버튼 클릭 시 onClose가 호출된다', async () => {
    (fetchRecommendationDetail as ReturnType<typeof vi.fn>).mockResolvedValue(mockDetail);
    const onClose = vi.fn();
    render(<StockDetail krxCode="005930" onClose={onClose} />);
    await waitFor(() => {
      expect(screen.getByText('005930')).toBeInTheDocument();
    });
