...
- **REQ-FOREX-031**: WHEN `calculate_performance()`가 `market='NYSE'` 또는 `'NASDAQ'` 종목을 처리할 때 THE 시스템 SHALL FDR `fdr.DataReader(ticker)`로 최신 종가(USD)를 조회한다.
- **REQ-FOREX-032**: THE 시스템 SHALL 해외 종목의 FDR 동기 호출을 `run_in_executor`로 격리한다.
...
- **REQ-FOREX-050**: WHEN `risk_analysis.py`의 상관관계·변동성 계산이 `market='NYSE'`/`'NASDAQ'` 종목을 포함할 때 THE 시스템 SHALL FDR `DataReader(ticker)` 경로로 해외 종가 시계열을 조회한다.
...
- **REQ-FOREX-080**: THE 시스템 SHALL 신규 마이그레이션 `0019_portfolio_foreign_asset.py`를 제공한다(`down_revision=0018`).
...
### AC-6: 해외 종목 성과 KRW 환산 (REQ-FOREX-031 ~ 036, 039)
...
### AC-11: 마이그레이션 0019 (REQ-FOREX-080 ~ 082)

WHEN `0019_portfolio_foreign_asset` 마이그레이션 `upgrade()`를 실행하면 THE 시스템 SHALL `portfolio_holdings` 테이블에 `market`(`DEFAULT 'KRX'`)·`currency`(`DEFAULT 'KRW'`) 컬럼을 추가하고, 기존 행들에 기본값을 적용하며, 고유 제약을 `(portfolio_id, krx_code, market)` 조합으로 변경한다. WHERE `downgrade()`가 실행되는 경우 THE 시스템 SHALL 두 컬럼을 제거하고 고유 제약을 원래대로 복원한다.
