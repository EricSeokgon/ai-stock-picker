...
interface DashboardSectorHeatmapProps {
  sectors: SectorItem[];
  loading: boolean;
}

// 수익률에 따른 색상 반환: 양수 → 초록, 음수 → 빨강
function getColor(returnPct: number): string {
  if (returnPct > 0) return '#22c55e';
  if (returnPct < 0) return '#ef4444';
  return '#94a3b8';
}

export default function DashboardSectorHeatmap({ sectors, loading }: DashboardSectorHeatmapProps) {
  if (loading) {
    return (
