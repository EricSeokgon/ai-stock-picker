...
export default function DashboardAssetAllocation({ assets, loading }) {
  if (loading) {
    return (
    ...
    <ResponsiveContainer width="100%" height={220}>
      <PieChart>
        <Pie
          ...
          outerRadius={80}
          dataKey="value"
          label={({ name, weight_pct }) => `${name} ${weight_pct.toFixed(1)}%`}
        >
        ...
        <Tooltip formatter={(value) => [`${value.toLocaleString()}원`, '평가액']} />
        <Legend />
      </PieChart>
    </ResponsiveContainer>
  );
